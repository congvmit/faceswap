...
def test_normalize_output_path() -> None:
    if platform.system().lower() == "linux" or platform.system().lower() == "darwin":
        assert (
            normalize_output_path(
                ".assets/examples/target-240p.mp4", ".assets/examples/target-240p.mp4"
            )
            == ".assets/examples/target-240p.mp4"
        )
        assert normalize_output_path(
            ".assets/examples/target-240p.mp4", ".assets/examples"
        ).startswith(".assets/examples/target-240p")
        assert normalize_output_path(
            ".assets/examples/target-240p.mp4", ".assets/examples"
        ).endswith(".mp4")
        assert (
            normalize_output_path(
                ".assets/examples/target-240p.mp4", ".assets/examples/output.mp4"
            )
            == ".assets/examples/output.mp4"
        )
    assert (
        normalize_output_path(
            ".assets/examples/target-240p.mp4", ".assets/examples/invalid"
        )
        is None
    )
    assert (
        normalize_output_path(
            ".assets/examples/target-240p.mp4", ".assets/invalid/output.mp4"
        )
        is None
    )
