...
def apply_execution_provider_options(execution_providers: List[str]) -> List[Any]:
    ...
    for execution_provider in execution_providers:
        if execution_provider == "CUDAExecutionProvider":
            execution_providers_with_options.append(
                (
                    execution_provider,
                    {
        ...
        execution_devices.append(
            {
                ...
                "product": {
                    "vendor": "NVIDIA",
                    "name": gpu_element.find("product_name").text.replace(
                        "NVIDIA ", ""
                    ),
                ...
                "video_memory": {
                    "total": create_value_and_unit(
                        gpu_element.find("fb_memory_usage/total").text
                    ),
                    "free": create_value_and_unit(
                        gpu_element.find("fb_memory_usage/free").text
                    ),
                },
                "utilization": {
                    "gpu": create_value_and_unit(
                        gpu_element.find("utilization/gpu_util").text
                    ),
                    "memory": create_value_and_unit(
                        gpu_element.find("utilization/memory_util").text
                    ),
                },
            }
        )
...
def create_value_and_unit(text: str) -> ValueAndUnit:
    ...
    value_and_unit: ValueAndUnit = {"value": value, "unit": unit}
