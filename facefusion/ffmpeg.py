...
def extract_frames(
    target_path: str, temp_video_resolution: str, temp_video_fps: Fps
) -> bool:
    ...
    if trim_frame_start is not None and trim_frame_end is not None:
        ...
        commands.extend(
            [
                "-vf",
                "scale=" + str(temp_video_resolution) + ",fps=" + str(temp_video_fps),
            ]
        )
    ...
    if facefusion.globals.output_video_encoder in ["libx264", "libx265"]:
        ...
        commands.extend(
            [
                ...
                "-preset",
                facefusion.globals.output_video_preset,
            ]
        )
    ...
    if facefusion.globals.output_video_encoder in ["h264_nvenc", "hevc_nvenc"]:
        ...
        commands.extend(
            [
                ...
                str(output_video_compression),
                "-preset",
                map_nvenc_preset(facefusion.globals.output_video_preset),
            ]
        )
...
def copy_image(target_path: str, output_path: str, temp_image_resolution: str) -> bool:
    ...
    commands = [
        ...
        target_path,
        "-q:v",
        str(temp_image_compression),
        "-vf",
        "scale=" + str(temp_image_resolution),
        "-y",
        output_path,
    ]
    return run_ffmpeg(commands)
...
def finalize_image(output_path: str, output_image_resolution: str) -> bool:
    ...
    commands = [
        ...
        "-vf",
        "scale=" + str(output_image_resolution),
        "-y",
        output_path,
    ]
    return run_ffmpeg(commands)
...
def read_audio_buffer(
    target_path: str, sample_rate: int, channel_total: int
