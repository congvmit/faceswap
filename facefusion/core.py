...
from facefusion.normalizer import (
    normalize_fps,
    normalize_output_path,
    normalize_padding,
)
from facefusion.processors.frame.core import (
...
def cli() -> None:
    ...
    program = ArgumentParser(
        formatter_class=lambda prog: HelpFormatter(prog, max_help_position=160),
        add_help=False,
    )
    # general
    ...
    group_misc.add_argument(
        "--skip-download",
        help=wording.get("help.skip_download"),
        action="store_true",
        default=config.get_bool_value("misc.skip_download"),
    )
    group_misc.add_argument(
        "--headless",
        help=wording.get("help.headless"),
        action="store_true",
        default=config.get_bool_value("misc.headless"),
    )
    group_misc.add_argument(
        "--log-level",
        help=wording.get("help.log_level"),
        default=config.get_str_value("misc.log_level", "info"),
        choices=logger.get_log_levels(),
    )
    # execution
    execution_providers = encode_execution_providers(
        onnxruntime.get_available_providers()
    )
    group_execution = program.add_argument_group("execution")
    group_execution.add_argument(
        "--execution-providers",
        help=wording.get("help.execution_providers").format(
            choices=", ".join(execution_providers)
        ),
        default=config.get_str_list("execution.execution_providers", "cpu"),
    ...
    group_execution.add_argument(
        ...
        help=wording.get("help.execution_thread_count"),
        type=int,
    ...
    group_execution.add_argument(
        ...
        help=wording.get("help.execution_queue_count"),
        type=int,
    ...
    group_face_analyser.add_argument(
        ...
        help=wording.get("help.face_landmarker_score"),
        type=float,
    ...
    group_face_mask.add_argument(
        "--face-mask-blur",
        help=wording.get("help.face_mask_blur"),
        type=float,
    ...
    group_face_mask.add_argument(
        "--face-mask-padding",
        help=wording.get("help.face_mask_padding"),
        type=int,
        default=config.get_int_list("face_mask.face_mask_padding", "0 0 0 0"),
        nargs="+",
    )
    group_face_mask.add_argument(
        "--face-mask-regions",
        help=wording.get("help.face_mask_regions").format(
            choices=", ".join(facefusion.choices.face_mask_regions)
        ),
        default=config.get_str_list(
            "face_mask.face_mask_regions",
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("help.output_image_quality"),
        type=int,
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("help.output_image_resolution"),
        default=config.get_str_value("output_creation.output_image_resolution"),
    )
    group_output_creation.add_argument(
        "--output-video-encoder",
        help=wording.get("help.output_video_encoder"),
    ...
    group_output_creation.add_argument(
        "--output-video-preset",
        help=wording.get("help.output_video_preset"),
    ...
    group_output_creation.add_argument(
        ...
        help=wording.get("help.output_video_quality"),
        type=int,
    ...
    group_frame_processors.add_argument(
        "--frame-processors",
        help=wording.get("help.frame_processors").format(
    ...
    group_uis.add_argument(
        "--ui-layouts",
        help=wording.get("help.ui_layouts").format(
...
def apply_args(program: ArgumentParser) -> None:
    ...
    if (
        args.face_detector_size
        in facefusion.choices.face_detector_set[args.face_detector_model]
    ):
    ...
    if is_video(args.target_path):
        ...
        if args.output_video_resolution in output_video_resolutions:
            ...
            facefusion.globals.output_video_resolution = pack_resolution(
                output_video_resolution
            )
    if args.output_video_fps or is_video(args.target_path):
        facefusion.globals.output_video_fps = normalize_fps(
            args.output_video_fps
        ) or detect_video_fps(args.target_path)
...
def run(program: ArgumentParser) -> None:
    ...
    if (
        ...
        or not voice_extractor.pre_check()
    ):
        return
...
def pre_check() -> bool:
    if sys.version_info < (3, 9):
        logger.error(
            wording.get("python_not_supported").format(version="3.9"), __name__.upper()
        )
        return False
...
def conditional_process_batch() -> None:
    start_time = time()

    output_progress_bar = get_ui_component("output_progress_bar")
    # output_progress_bar(0, desc="Processing...")

    for target_path in output_progress_bar.tqdm(
        facefusion.globals.target_paths, desc="Processing..."
    ):
        facefusion.globals.target_path = target_path["path"]
        facefusion.globals.target_type = target_path["type"]
        print("Processing Target Path: ", facefusion.globals.target_path)
        print("Processing Source Path: ", facefusion.globals.source_paths)

        for frame_processor_module in get_frame_processors_modules(
            facefusion.globals.frame_processors
        ):
            while not frame_processor_module.post_check():
                print("Batch: Frame Processor Post Check Failed")
                logger.disable()
                sleep(0.5)
            logger.enable()
            if not frame_processor_module.pre_process("output"):
                print("Batch: Frame Processor Pre Process Failed")
                return
        ...
        if is_image(facefusion.globals.target_path):
            ...
            facefusion.globals.output_image_resolutions = output_image_resolutions
        ...
        if is_video(facefusion.globals.target_path):
            ...
            facefusion.globals.output_video_resolutions = output_video_resolutions
...
def conditional_append_reference_faces() -> None:
    if (
        "reference" in facefusion.globals.face_selector_mode
        ...
        reference_face = get_one_face(
            reference_frame, facefusion.globals.reference_face_position
        )
            ...
            for frame_processor_module in get_frame_processors_modules(
                ...
                if numpy.any(abstract_reference_frame):
                    ...
                    append_reference_face(
                        frame_processor_module.__name__, reference_face
                    )

        print("Reference face appended!")
...
def process_image(start_time: float) -> None:
    ...
    temp_image_resolution = pack_resolution(
        restrict_image_resolution(
            facefusion.globals.target_path,
            unpack_resolution(facefusion.globals.output_image_resolution),
        )
    )
    logger.info(
        wording.get("copying_image").format(resolution=temp_image_resolution),
        __name__.upper(),
    )
    if copy_image(
        facefusion.globals.target_path, normed_output_path, temp_image_resolution
    ):
    ...
    for frame_processor_module in get_frame_processors_modules(
        ...
        frame_processor_module.process_image(
            facefusion.globals.source_paths, normed_output_path, normed_output_path
        )
    ...
    logger.info(
        wording.get("finalizing_image").format(
            resolution=facefusion.globals.output_image_resolution
        ),
        __name__.upper(),
    )
    ...
    if is_image(normed_output_path):
        ...
        logger.info(
            wording.get("processing_image_succeed").format(seconds=seconds),
            __name__.upper(),
        )
...
def process_video(start_time: float) -> None:
    ...
    if analyse_video(
        facefusion.globals.target_path,
        facefusion.globals.trim_frame_start,
    ...
    temp_video_resolution = pack_resolution(
        restrict_video_resolution(
            facefusion.globals.target_path,
            unpack_resolution(facefusion.globals.output_video_resolution),
        )
    )
    temp_video_fps = restrict_video_fps(
        facefusion.globals.target_path, facefusion.globals.output_video_fps
    )
    logger.info(
    ...
    if extract_frames(
        facefusion.globals.target_path, temp_video_resolution, temp_video_fps
    ):
    ...
    if temp_frame_paths:
        for frame_processor_module in get_frame_processors_modules(
            ...
            logger.info(wording.get("processing"), frame_processor_module.NAME)
            frame_processor_module.process_video(
                facefusion.globals.source_paths, temp_frame_paths
            )
            frame_processor_module.post_process()
        if is_process_stopping():
            return
    ...
    logger.info(
        wording.get("merging_video").format(
            resolution=facefusion.globals.output_video_resolution,
    ...
    if facefusion.globals.skip_audio:
        ...
        if "lip_syncer" in facefusion.globals.frame_processors:
            source_audio_path = get_first(
                filter_audio_paths(facefusion.globals.source_paths)
            )
            if source_audio_path and replace_audio(
                facefusion.globals.target_path, source_audio_path, normed_output_path
            ):
    ...
    if is_video(normed_output_path):
        ...
        logger.info(
            wording.get("processing_video_succeed").format(seconds=seconds),
            __name__.upper(),
        )
        conditional_log_statistics()
    else:
...
def process_batch(start_time: float) -> None:
    # TODO: Implement video and image batch processing

    if len(facefusion.globals.target_paths) == 0:
        raise ValueError("No target paths provided")

    normed_output_path = normalize_output_path(
        facefusion.globals.target_paths[0], facefusion.globals.output_path
    )

    from tqdm import tqdm

    for i, target_item in tqdm(enumerate(facefusion.globals.target_paths)):
        ...
        temp_video_fps = restrict_video_fps(
            target_path, facefusion.globals.output_video_fps
        )
        logger.info(
            wording.get("extracting_frames").format(
                resolution=temp_video_resolution, fps=temp_video_fps
        ...
        if facefusion.globals.skip_audio:
            ...
            if "lip_syncer" in facefusion.globals.frame_processors:
                ...
                if source_audio_path and replace_audio(
                    target_path, source_audio_path, normed_output_path
                ):
                    logger.debug(
                        wording.get("restoring_audio_succeed"), __name__.upper()
                    )
                else:
                    if is_process_stopping():
                        return
                    logger.warn(
                        wording.get("restoring_audio_skipped"), __name__.upper()
                    )
                    move_temp(target_path, normed_output_path)
            else:
                if restore_audio(
