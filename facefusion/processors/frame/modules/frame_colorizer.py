...
def get_options(key: Literal["model"]) -> Any:
    ...
    if OPTIONS is None:
        OPTIONS = {"model": MODELS[frame_processors_globals.frame_colorizer_model]}
...
def register_args(program: ArgumentParser) -> None:
    program.add_argument(
        ...
        help=wording.get("help.frame_colorizer_model"),
        default=config.get_str_value(
            "frame_processors.frame_colorizer_model", "ddcolor"
        ),
        choices=frame_processors_choices.frame_colorizer_models,
    )
    program.add_argument(
        "--frame-colorizer-blend",
        help=wording.get("help.frame_colorizer_blend"),
        type=int,
        default=config.get_int_value("frame_processors.frame_colorizer_blend", "100"),
        choices=frame_processors_choices.frame_colorizer_blend_range,
        metavar=create_metavar(frame_processors_choices.frame_colorizer_blend_range),
    )
...
def merge_color_frame(
    temp_vision_frame: VisionFrame, color_vision_frame: VisionFrame
) -> VisionFrame:
    model_type = get_options("model").get("type")
    ...
    if model_type == "deoldify":
        ...
        color_vision_frame = cv2.cvtColor(color_vision_frame, cv2.COLOR_BGR2RGB).astype(
            numpy.uint8
        )
...
def blend_frame(
    temp_vision_frame: VisionFrame, paste_vision_frame: VisionFrame
) -> VisionFrame:
