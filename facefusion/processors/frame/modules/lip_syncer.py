...
from facefusion import config, logger, process_manager, wording
from facefusion.audio import (
    create_empty_audio_frame,
    get_voice_frame,
    read_static_voice,
)
...
def get_options(key: Literal["model"]) -> Any:
    ...
    if OPTIONS is None:
        OPTIONS = {"model": MODELS[frame_processors_globals.lip_syncer_model]}
...
def register_args(program: ArgumentParser) -> None:
    program.add_argument(
        "--lip-syncer-model",
        help=wording.get("help.lip_syncer_model"),
        default=config.get_str_value(
            "frame_processors.lip_syncer_model", "wav2lip_gan"
        ),
...
def post_check() -> bool:
    ...
    if not facefusion.globals.skip_download and not is_download_done(
        model_url, model_path
    ):
        logger.error(
            wording.get("model_download_not_done") + wording.get("exclamation_mark"),
            NAME,
        )
        return False
    if not is_file(model_path):
        logger.error(
            wording.get("model_file_not_present") + wording.get("exclamation_mark"),
            NAME,
        )
        return False
    return True
...
def pre_process(mode: ProcessMode) -> bool:
    if not has_audio(facefusion.globals.source_paths):
        logger.error(
            wording.get("select_audio_source") + wording.get("exclamation_mark"), NAME
        )
        return False
    if (
        mode in ["output", "preview"]
        and not is_image(facefusion.globals.target_path)
        and not is_video(facefusion.globals.target_path)
    ):
        logger.error(
            wording.get("select_image_or_video_target")
            + wording.get("exclamation_mark"),
            NAME,
        )
        return False
    if mode == "output" and not normalize_output_path(
        facefusion.globals.target_path, facefusion.globals.output_path
    ):
        logger.error(
            wording.get("select_file_or_directory_output")
            + wording.get("exclamation_mark"),
            NAME,
        )
        return False
...
def post_process() -> None:
    ...
    if (
        ...
        or facefusion.globals.video_memory_strategy == "moderate"
    ):
...
def sync_lip(
    target_face: Face, temp_audio_frame: AudioFrame, temp_vision_frame: VisionFrame
) -> VisionFrame:
    frame_processor = get_frame_processor()
    crop_mask_list = []
    temp_audio_frame = prepare_audio_frame(temp_audio_frame)
    crop_vision_frame, affine_matrix = warp_face_by_face_landmark_5(
        temp_vision_frame, target_face.landmarks.get("5/68"), "ffhq_512", (512, 512)
    )
    face_landmark_68 = cv2.transform(
        target_face.landmarks.get("68").reshape(1, -1, 2), affine_matrix
    ).reshape(-1, 2)
    ...
    close_vision_frame, close_matrix = warp_face_by_bounding_box(
        crop_vision_frame, bounding_box, (96, 96)
    )
    close_vision_frame = prepare_crop_frame(close_vision_frame)
    close_vision_frame = frame_processor.run(
        None, {"source": temp_audio_frame, "target": close_vision_frame}
    )[0]
    crop_vision_frame = normalize_crop_frame(close_vision_frame)
    crop_vision_frame = cv2.warpAffine(
        crop_vision_frame,
        cv2.invertAffineTransform(close_matrix),
        (512, 512),
...
def prepare_crop_frame(crop_vision_frame: VisionFrame) -> VisionFrame:
    ...
    crop_vision_frame = numpy.concatenate(
        (prepare_vision_frame, crop_vision_frame), axis=3
    )
    crop_vision_frame = (
        crop_vision_frame.transpose(0, 3, 1, 2).astype("float32") / 255.0
    )
    ...
    if facefusion.globals.face_selector_mode == "many":
        ...
        if many_faces:
            for target_face in many_faces:
                target_vision_frame = sync_lip(
                    target_face, source_audio_frame, target_vision_frame
                )
    ...
    if facefusion.globals.face_selector_mode == "reference":
        ...
        if similar_faces:
            for similar_face in similar_faces:
                target_vision_frame = sync_lip(
                    similar_face, source_audio_frame, target_vision_frame
                )
...
def process_frames(
    ...
    for queue_payload in process_manager.manage(queue_payloads):
        ...
        source_audio_frame = get_voice_frame(
...
def process_video(source_paths: List[str], temp_frame_paths: List[str]) -> None:
    frame_processors.multi_process_frames(
        source_paths, temp_frame_paths, process_frames
    )
