...
def get_options(key: Literal["model"]) -> Any:
    ...
    if OPTIONS is None:
        OPTIONS = {"model": MODELS[frame_processors_globals.frame_enhancer_model]}
...
def register_args(program: ArgumentParser) -> None:
    program.add_argument(
        ...
        help=wording.get("help.frame_enhancer_model"),
        default=config.get_str_value(
            "frame_processors.frame_enhancer_model", "span_kendata_x4"
        ),
        choices=frame_processors_choices.frame_enhancer_models,
    )
    program.add_argument(
        "--frame-enhancer-blend",
        help=wording.get("help.frame_enhancer_blend"),
        type=int,
...
def blend_frame(
    temp_vision_frame: VisionFrame, merge_vision_frame: VisionFrame
) -> VisionFrame:
