...
def get_options(key: Literal["model"]) -> Any:
    ...
    if OPTIONS is None:
        OPTIONS = {"model": MODELS[frame_processors_globals.face_swapper_model]}
...
def register_args(program: ArgumentParser) -> None:
    ...
    program.add_argument(
        "--face-swapper-model",
        help=wording.get("help.face_swapper_model"),
        default=config.get_str_value(
            "frame_processors.face_swapper_model", face_swapper_model_fallback
        ),
...
def apply_args(program: ArgumentParser) -> None:
    ...
    if (
        args.face_swapper_model == "inswapper_128"
        or args.face_swapper_model == "inswapper_128_fp16"
    ):
        facefusion.globals.face_recognizer_model = "arcface_inswapper"
    if (
        args.face_swapper_model == "simswap_256"
        or args.face_swapper_model == "simswap_512_unofficial"
    ):
...
def pre_process(mode: ProcessMode) -> bool:
    if not has_image(facefusion.globals.source_paths):
        logger.error(
            wording.get("select_image_source") + wording.get("exclamation_mark"), NAME
        )
        return False
    ...
    for source_frame in source_frames:
        if not get_one_face(source_frame):
            logger.error(
                wording.get("no_source_face_detected")
...
def swap_face(
    ...
    crop_vision_frame, affine_matrix = warp_face_by_face_landmark_5(
        temp_vision_frame, target_face.landmarks.get("5/68"), model_template, model_size
    )
    crop_mask_list = []
    ...
    if "region" in facefusion.globals.face_mask_types:
        region_mask = create_region_mask(
            crop_vision_frame, facefusion.globals.face_mask_regions
        )
    ...
    for frame_processor_input in frame_processor.get_inputs():
        if frame_processor_input.name == "source":
            if model_type == "blendswap" or model_type == "uniface":
                ...
                frame_processor_inputs[frame_processor_input.name] = (
                    prepare_source_embedding(source_face)
                )
...
def prepare_source_frame(source_face: Face) -> VisionFrame:
    ...
    if model_type == "blendswap":
        source_vision_frame, _ = warp_face_by_face_landmark_5(
            source_vision_frame,
            source_face.landmarks.get("5/68"),
            "arcface_112_v2",
            (112, 112),
        )
    ...
    source_vision_frame = numpy.expand_dims(source_vision_frame, axis=0).astype(
        numpy.float32
    )
        ...
        if many_faces:
            for target_face in many_faces:
                target_vision_frame = swap_face(
                    source_face, target_face, target_vision_frame
                )
