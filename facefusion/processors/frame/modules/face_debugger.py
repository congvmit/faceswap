...
def register_args(program: ArgumentParser) -> None:
    program.add_argument(
        ...
        default=config.get_str_list(
            "frame_processors.face_debugger_items", "face-landmark-5/68 face-mask"
        ),
...
def debug_face(target_face: Face, temp_vision_frame: VisionFrame) -> VisionFrame:
    ...
    has_face_landmark_5_fallback = numpy.array_equal(
        target_face.landmarks.get("5"), target_face.landmarks.get("5/68")
    )
    has_face_landmark_68_fallback = numpy.array_equal(
        target_face.landmarks.get("68"), target_face.landmarks.get("68/5")
    )
    ...
    if "face-mask" in frame_processors_globals.face_debugger_items:
        ...
        cv2.drawContours(
            ...
            inverse_contours,
            -1,
            tertiary_color if has_face_landmark_5_fallback else secondary_color,
            2,
        )
    if "face-landmark-5" in frame_processors_globals.face_debugger_items and numpy.any(
        ...
        for index in range(face_landmark_5.shape[0]):
            cv2.circle(
                temp_vision_frame,
                (face_landmark_5[index][0], face_landmark_5[index][1]),
                3,
                primary_color,
                -1,
            )
    if (
        "face-landmark-5/68" in frame_processors_globals.face_debugger_items
        and numpy.any(target_face.landmarks.get("5/68"))
    ):
        face_landmark_5_68 = target_face.landmarks.get("5/68").astype(numpy.int32)
        for index in range(face_landmark_5_68.shape[0]):
            cv2.circle(
                temp_vision_frame,
                (face_landmark_5_68[index][0], face_landmark_5_68[index][1]),
                3,
                tertiary_color if has_face_landmark_5_fallback else secondary_color,
                -1,
            )
    if (
        "face-landmark-68" in frame_processors_globals.face_debugger_items
        and numpy.any(target_face.landmarks.get("68"))
    ):
        face_landmark_68 = target_face.landmarks.get("68").astype(numpy.int32)
        for index in range(face_landmark_68.shape[0]):
            cv2.circle(
                temp_vision_frame,
                (face_landmark_68[index][0], face_landmark_68[index][1]),
                3,
                tertiary_color if has_face_landmark_68_fallback else secondary_color,
                -1,
            )
    if (
        "face-landmark-68/5" in frame_processors_globals.face_debugger_items
        and numpy.any(target_face.landmarks.get("68"))
    ):
    ...
    if (
        ...
        and bounding_box[2] - bounding_box[0] > 50
    ):
        top = bounding_box[1]
...
def process_image(source_paths: List[str], target_path: str, output_path: str) -> None:
    ...
    output_vision_frame = process_frame(
        {"reference_faces": reference_faces, "target_vision_frame": target_vision_frame}
    )
