...
def get_options(key: Literal["model"]) -> Any:
    ...
    if OPTIONS is None:
        OPTIONS = {"model": MODELS[frame_processors_globals.face_enhancer_model]}
...
def register_args(program: ArgumentParser) -> None:
    program.add_argument(
        "--face-enhancer-model",
        help=wording.get("help.face_enhancer_model"),
        default=config.get_str_value(
            "frame_processors.face_enhancer_model", "gfpgan_1.4"
        ),
        choices=frame_processors_choices.face_enhancer_models,
    )
    program.add_argument(
        "--face-enhancer-blend",
        help=wording.get("help.face_enhancer_blend"),
        type=int,
