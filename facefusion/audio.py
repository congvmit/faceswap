...
def get_audio_frame(
    audio_path: str, fps: Fps, frame_number: int = 0
) -> Optional[AudioFrame]:
    if is_audio(audio_path):
...
def create_mel_filter_bank() -> MelFilterBank:
    ...
    mel_frequency_range = numpy.linspace(
        convert_hertz_to_mel(min_frequency),
        convert_hertz_to_mel(max_frequency),
        mel_filter_total + 2,
    )
    indices = numpy.floor(
        (mel_bin_total + 1) * convert_mel_to_hertz(mel_frequency_range) / sample_rate
    ).astype(numpy.int16)
...
def create_spectrogram(audio: Audio) -> Spectrogram:
    ...
    spectrogram = scipy.signal.stft(
        audio, nperseg=mel_bin_total, nfft=mel_bin_total, noverlap=mel_bin_overlap
    )[2]
...
def extract_audio_frames(spectrogram: Spectrogram, fps: Fps) -> List[AudioFrame]:
    ...
    step_size = 16
    audio_frames = []
    indices = numpy.arange(0, spectrogram.shape[1], mel_filter_total / fps).astype(
        numpy.int16
    )
