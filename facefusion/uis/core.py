...
from facefusion import logger, metadata, wording
from facefusion.filesystem import resolve_relative_path
from facefusion.uis import colors, overrides
...
UI_COMPONENTS: Dict[ComponentName, Component] = {}  # type: ignore
...
def load_ui_layout_module(ui_layout: str) -> Any:
    try:
        ui_layout_module = importlib.import_module(
            "facefusion.uis.layouts." + ui_layout
        )
        ...
        logger.error(
            wording.get("ui_layout_not_loaded").format(ui_layout=ui_layout),
            __name__.upper(),
        )
        ...
        logger.error(
            wording.get("ui_layout_not_implemented").format(ui_layout=ui_layout),
            __name__.upper(),
        )
        sys.exit(1)
...
def get_ui_component(component_name: ComponentName) -> Optional[Component]:  # type: ignore
