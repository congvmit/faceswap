...
def render() -> None:
    ...
    if is_video(facefusion.globals.target_path):
        ...
        trim_frame_end_slider_args["value"] = (
            facefusion.globals.trim_frame_end or video_frame_total
        )
...
def listen() -> None:
    TRIM_FRAME_START_SLIDER.release(
        update_trim_frame_start, inputs=TRIM_FRAME_START_SLIDER
    )
    ...
    if target_video:
        for method in ["upload", "change", "clear"]:
            getattr(target_video, method)(
                remote_update, outputs=[TRIM_FRAME_START_SLIDER, TRIM_FRAME_END_SLIDER]
            )
...
def remote_update() -> Tuple[gradio.Slider, gradio.Slider]:
    if is_video(facefusion.globals.target_path):
        ...
        return gradio.Slider(
            value=0, maximum=video_frame_total, visible=True
        ), gradio.Slider(
            value=video_frame_total, maximum=video_frame_total, visible=True
        )
    return gradio.Slider(value=None, maximum=None, visible=False), gradio.Slider(
        value=None, maximum=None, visible=False
    )


def update_trim_frame_start(trim_frame_start: int) -> None:
    facefusion.globals.trim_frame_start = (
        trim_frame_start if trim_frame_start > 0 else None
    )
...
def update_trim_frame_end(trim_frame_end: int) -> None:
    ...
    facefusion.globals.trim_frame_end = (
        trim_frame_end if trim_frame_end < video_frame_total else None
    )
