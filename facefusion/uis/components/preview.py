...
def render() -> None:
    ...
    if (
        source_audio_path
        and facefusion.globals.output_video_fps
        and facefusion.globals.reference_frame_number
    ):
        temp_audio_frame = get_audio_frame(
            source_audio_path,
    ...
    if is_image(facefusion.globals.target_path):
        ...
        preview_vision_frame = process_preview_frame(
            reference_faces, source_face, source_audio_frame, target_vision_frame
        )
        preview_image_args["value"] = normalize_frame_color(preview_vision_frame)
    if is_video(facefusion.globals.target_path):
        temp_vision_frame = get_video_frame(
            facefusion.globals.target_path, facefusion.globals.reference_frame_number
        )
        preview_vision_frame = process_preview_frame(
            reference_faces, source_face, source_audio_frame, temp_vision_frame
        )
        preview_image_args["value"] = normalize_frame_color(preview_vision_frame)
        preview_image_args["visible"] = True
        preview_frame_slider_args["value"] = facefusion.globals.reference_frame_number
        preview_frame_slider_args["maximum"] = count_video_frame_total(
...
def listen() -> None:
    ...
    for ui_component in get_ui_components(["target_image", "target_video"]):
        for method in ["upload", "change", "clear"]:
            getattr(ui_component, method)(
                update_preview_frame_slider, outputs=PREVIEW_FRAME_SLIDER
            )
    ...
    for ui_component in get_ui_components(
        ["face_detector_score_slider", "face_landmarker_score_slider"]
    ):
        ui_component.release(
            clear_and_update_preview_image,
            inputs=PREVIEW_FRAME_SLIDER,
...
def update_preview_image(frame_number: int = 0) -> gradio.Image:
    ...
    if is_video(facefusion.globals.target_path):
        temp_vision_frame = get_video_frame(
            facefusion.globals.target_path, frame_number
        )
        preview_vision_frame = process_preview_frame(
            reference_faces, source_face, source_audio_frame, temp_vision_frame
        )
