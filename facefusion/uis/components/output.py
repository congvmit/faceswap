...
def render() -> None:
    ...
    OUTPUT_IMAGE = gradio.Image(
        label=wording.get("uis.output_image_or_video"), visible=False
    )
    OUTPUT_VIDEO = gradio.Video(label=wording.get("uis.output_image_or_video"))
...
def listen() -> None:
    ...
    if output_path_textbox:
        OUTPUT_START_BUTTON.click(
            start, outputs=[OUTPUT_START_BUTTON, OUTPUT_STOP_BUTTON]
        )
        OUTPUT_START_BUTTON.click(
            process,
            outputs=[
...
def process() -> Tuple[gradio.Image, gradio.Video, gradio.Button, gradio.Button]:
    ...
    if is_image(normed_output_path):
        return (
            gradio.Image(value=normed_output_path, visible=True),
    ...
    if is_video(normed_output_path):
        return (
            ...
            gradio.Video(value=normed_output_path, visible=True),
            gradio.Button(visible=True),
            gradio.Button(visible=False),
        )
    return (
