...
def render() -> None:
    ...
    SOURCE_FILE = gradio.File(
        file_count="multiple",
        file_types=[".mp3", ".wav", ".png", ".jpg", ".webp"],
        label=wording.get("uis.source_file"),
        value=facefusion.globals.source_paths
        if has_source_audio or has_source_image
        else None,
    )
    source_file_names = (
        [source_file_value["name"] for source_file_value in SOURCE_FILE.value]
...
def update(files: List[File]) -> Tuple[gradio.Audio, gradio.Image]:
    ...
    if has_source_audio or has_source_image:
        ...
        return gradio.Audio(
            value=source_audio_path, visible=has_source_audio
        ), gradio.Image(value=source_image_path, visible=has_source_image)
    facefusion.globals.source_paths = None
    return gradio.Audio(value=None, visible=False), gradio.Image(
