...
def render() -> None:
    ...
    with gradio.Group(visible=has_box_mask) as FACE_MASK_BOX_GROUP:
        FACE_MASK_BLUR_SLIDER = gradio.Slider(
            ...
            step=facefusion.choices.face_mask_blur_range[1]
            - facefusion.choices.face_mask_blur_range[0],
        ...
        with gradio.Row():
            FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
                ...
                step=facefusion.choices.face_mask_padding_range[1]
                - facefusion.choices.face_mask_padding_range[0],
...
def listen() -> None:
    FACE_MASK_TYPES_CHECKBOX_GROUP.change(
        update_face_mask_type,
    ...
    FACE_MASK_REGION_CHECKBOX_GROUP.change(
        update_face_mask_regions,
        inputs=FACE_MASK_REGION_CHECKBOX_GROUP,
    ...
    for face_mask_padding_slider in face_mask_padding_sliders:
        face_mask_padding_slider.release(
            update_face_mask_padding, inputs=face_mask_padding_sliders
        )


def update_face_mask_type(
    face_mask_types: List[FaceMaskType],
) -> Tuple[gradio.CheckboxGroup, gradio.Group, gradio.CheckboxGroup]:
    facefusion.globals.face_mask_types = (
        face_mask_types or facefusion.choices.face_mask_types
    )
    has_box_mask = "box" in face_mask_types
    has_region_mask = "region" in face_mask_types
    return (
        gradio.CheckboxGroup(value=facefusion.globals.face_mask_types),
        gradio.Group(visible=has_box_mask),
        gradio.CheckboxGroup(visible=has_region_mask),
    )
...
def update_face_mask_padding(
    face_mask_padding_top: int,
    face_mask_padding_right: int,
    face_mask_padding_bottom: int,
    face_mask_padding_left: int,
) -> None:
...
def update_face_mask_regions(
    face_mask_regions: List[FaceMaskRegion],
) -> gradio.CheckboxGroup:
    facefusion.globals.face_mask_regions = (
        face_mask_regions or facefusion.choices.face_mask_regions
    )
