...
from facefusion.processors.frame.typings import (
    FaceDebuggerItem,
    FaceEnhancerModel,
    FaceSwapperModel,
    FrameColorizerModel,
...
def render() -> None:
    ...
    FACE_ENHANCER_BLEND_SLIDER = gradio.Slider(
        ...
        step=frame_processors_choices.face_enhancer_blend_range[1]
        - frame_processors_choices.face_enhancer_blend_range[0],
    ...
    FRAME_COLORIZER_BLEND_SLIDER = gradio.Slider(
        ...
        step=frame_processors_choices.frame_colorizer_blend_range[1]
        - frame_processors_choices.frame_colorizer_blend_range[0],
...
def listen() -> None:
    FACE_DEBUGGER_ITEMS_CHECKBOX_GROUP.change(
        update_face_debugger_items, inputs=FACE_DEBUGGER_ITEMS_CHECKBOX_GROUP
    )
    FACE_ENHANCER_MODEL_DROPDOWN.change(
        update_face_enhancer_model,
        inputs=FACE_ENHANCER_MODEL_DROPDOWN,
        outputs=FACE_ENHANCER_MODEL_DROPDOWN,
    )
    FACE_ENHANCER_BLEND_SLIDER.release(
        update_face_enhancer_blend, inputs=FACE_ENHANCER_BLEND_SLIDER
    )
    ...
    FRAME_COLORIZER_MODEL_DROPDOWN.change(
        update_frame_colorizer_model,
        inputs=FRAME_COLORIZER_MODEL_DROPDOWN,
        outputs=FRAME_COLORIZER_MODEL_DROPDOWN,
    )
    FRAME_COLORIZER_BLEND_SLIDER.release(
        update_frame_colorizer_blend, inputs=FRAME_COLORIZER_BLEND_SLIDER
    )
    FRAME_ENHANCER_MODEL_DROPDOWN.change(
        update_frame_enhancer_model,
        inputs=FRAME_ENHANCER_MODEL_DROPDOWN,
        outputs=FRAME_ENHANCER_MODEL_DROPDOWN,
    )
    FRAME_ENHANCER_BLEND_SLIDER.release(
        update_frame_enhancer_blend, inputs=FRAME_ENHANCER_BLEND_SLIDER
    )
    LIP_SYNCER_MODEL_DROPDOWN.change(
        update_lip_syncer_model,
        inputs=LIP_SYNCER_MODEL_DROPDOWN,
        outputs=LIP_SYNCER_MODEL_DROPDOWN,
    )
    frame_processors_checkbox_group = get_ui_component(
        "frame_processors_checkbox_group"
    )
    if frame_processors_checkbox_group:
        frame_processors_checkbox_group.change(
            update_frame_processors,
...
def update_frame_processors(
    ...
    return (
        gradio.CheckboxGroup(visible=has_face_debugger),
        gradio.Dropdown(visible=has_face_enhancer),
        gradio.Slider(visible=has_face_enhancer),
        gradio.Dropdown(visible=has_face_swapper),
        gradio.Dropdown(visible=has_frame_colorizer),
        gradio.Slider(visible=has_frame_colorizer),
        gradio.Dropdown(visible=has_frame_enhancer),
        gradio.Slider(visible=has_frame_enhancer),
        gradio.Dropdown(visible=has_lip_syncer),
    )
...
def update_face_enhancer_model(
    ...
    face_enhancer_module.set_options(
        "model", face_enhancer_module.MODELS[face_enhancer_model]
    )
...
def update_face_swapper_model(face_swapper_model: FaceSwapperModel) -> gradio.Dropdown:
    ...
    if (
        ...
        or face_swapper_model == "inswapper_128_fp16"
    ):
        facefusion.globals.face_recognizer_model = "arcface_inswapper"
    if (
        face_swapper_model == "simswap_256"
        or face_swapper_model == "simswap_512_unofficial"
    ):
    ...
    face_swapper_module.set_options(
        "model", face_swapper_module.MODELS[face_swapper_model]
    )
...
def update_frame_colorizer_model(
    ...
    frame_colorizer_module.set_options(
        "model", frame_colorizer_module.MODELS[frame_colorizer_model]
    )
...
def update_frame_enhancer_model(
    ...
    frame_enhancer_module.set_options(
        "model", frame_enhancer_module.MODELS[frame_enhancer_model]
    )
