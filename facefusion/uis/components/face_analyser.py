...
def render() -> None:
    ...
    if (
        facefusion.globals.face_detector_size
        in facefusion.choices.face_detector_set[facefusion.globals.face_detector_model]
    ):
        face_detector_size_dropdown_args["choices"] = (
            facefusion.choices.face_detector_set[facefusion.globals.face_detector_model]
        )
    with gradio.Row():
    ...
    with gradio.Row():
        FACE_DETECTOR_SCORE_SLIDER = gradio.Slider(
            ...
            step=facefusion.choices.face_detector_score_range[1]
            - facefusion.choices.face_detector_score_range[0],
        ...
        FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
            ...
            step=facefusion.choices.face_landmarker_score_range[1]
            - facefusion.choices.face_landmarker_score_range[0],
...
def listen() -> None:
    FACE_ANALYSER_ORDER_DROPDOWN.change(
        update_face_analyser_order, inputs=FACE_ANALYSER_ORDER_DROPDOWN
    )
    FACE_ANALYSER_AGE_DROPDOWN.change(
        update_face_analyser_age, inputs=FACE_ANALYSER_AGE_DROPDOWN
    )
    FACE_ANALYSER_GENDER_DROPDOWN.change(
        update_face_analyser_gender, inputs=FACE_ANALYSER_GENDER_DROPDOWN
    )
    FACE_DETECTOR_MODEL_DROPDOWN.change(
        update_face_detector_model,
        inputs=FACE_DETECTOR_MODEL_DROPDOWN,
        outputs=[FACE_DETECTOR_MODEL_DROPDOWN, FACE_DETECTOR_SIZE_DROPDOWN],
    )
    FACE_DETECTOR_SIZE_DROPDOWN.change(
        update_face_detector_size, inputs=FACE_DETECTOR_SIZE_DROPDOWN
    )
    FACE_DETECTOR_SCORE_SLIDER.release(
        update_face_detector_score, inputs=FACE_DETECTOR_SCORE_SLIDER
    )
    FACE_LANDMARKER_SCORE_SLIDER.release(
        update_face_landmarker_score, inputs=FACE_LANDMARKER_SCORE_SLIDER
    )


def update_face_analyser_order(face_analyser_order: FaceAnalyserOrder) -> None:
    facefusion.globals.face_analyser_order = (
        face_analyser_order if face_analyser_order != "none" else None
    )


def update_face_analyser_age(face_analyser_age: FaceAnalyserAge) -> None:
    facefusion.globals.face_analyser_age = (
        face_analyser_age if face_analyser_age != "none" else None
    )


def update_face_analyser_gender(face_analyser_gender: FaceAnalyserGender) -> None:
    facefusion.globals.face_analyser_gender = (
        face_analyser_gender if face_analyser_gender != "none" else None
    )


def update_face_detector_model(
    face_detector_model: FaceDetectorModel,
) -> Tuple[gradio.Dropdown, gradio.Dropdown]:
    facefusion.globals.face_detector_model = face_detector_model
    update_face_detector_size("640x640")
    if face_analyser.pre_check():
        if (
            facefusion.globals.face_detector_size
            in facefusion.choices.face_detector_set[face_detector_model]
        ):
            return gradio.Dropdown(
                value=facefusion.globals.face_detector_model
            ), gradio.Dropdown(
                value=facefusion.globals.face_detector_size,
                choices=facefusion.choices.face_detector_set[face_detector_model],
            )
        return gradio.Dropdown(
            value=facefusion.globals.face_detector_model
        ), gradio.Dropdown(
            value=facefusion.globals.face_detector_size,
            choices=[facefusion.globals.face_detector_size],
        )
