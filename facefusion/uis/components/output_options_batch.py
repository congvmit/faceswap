...
def render() -> None:
    ...
    OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
        ...
        step=facefusion.choices.output_image_quality_range[1]
        - facefusion.choices.output_image_quality_range[0],
        minimum=facefusion.choices.output_image_quality_range[0],
        maximum=facefusion.choices.output_image_quality_range[-1],
        visible=True,  # is_image(facefusion.globals.target_path),
    )
    ...
    OUTPUT_VIDEO_ENCODER_DROPDOWN = gradio.Dropdown(
        ...
        visible=True,  # is_video(facefusion.globals.target_path),
    )
    ...
    OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
        ...
        step=facefusion.choices.output_video_quality_range[1]
        - facefusion.choices.output_video_quality_range[0],
    ...
    OUTPUT_VIDEO_RESOLUTION_DROPDOWN = gradio.Dropdown(
        ...
        visible=False,  # is_video(facefusion.globals.target_path),
    )
...
def listen() -> None:
    ...
    OUTPUT_IMAGE_QUALITY_SLIDER.release(
        update_output_image_quality, inputs=OUTPUT_IMAGE_QUALITY_SLIDER
    )
    OUTPUT_IMAGE_RESOLUTION_DROPDOWN.change(
        update_output_image_resolution, inputs=OUTPUT_IMAGE_RESOLUTION_DROPDOWN
    )
    OUTPUT_VIDEO_ENCODER_DROPDOWN.change(
        update_output_video_encoder, inputs=OUTPUT_VIDEO_ENCODER_DROPDOWN
    )
    OUTPUT_VIDEO_PRESET_DROPDOWN.change(
        update_output_video_preset, inputs=OUTPUT_VIDEO_PRESET_DROPDOWN
    )
    OUTPUT_VIDEO_QUALITY_SLIDER.release(
        update_output_video_quality, inputs=OUTPUT_VIDEO_QUALITY_SLIDER
    )
    OUTPUT_VIDEO_RESOLUTION_DROPDOWN.change(
        update_output_video_resolution, inputs=OUTPUT_VIDEO_RESOLUTION_DROPDOWN
    )
    OUTPUT_VIDEO_FPS_SLIDER.release(
        update_output_video_fps, inputs=OUTPUT_VIDEO_FPS_SLIDER
    )
...
def remote_update() -> Tuple[
    ...
    if is_image(facefusion.globals.target_path):
        ...
        return (
            gradio.Slider(visible=True),
            gradio.Dropdown(
                visible=True,
                value=facefusion.globals.output_image_resolution,
                choices=output_image_resolutions,
            ),
            gradio.Dropdown(visible=False),
            gradio.Dropdown(visible=False),
            gradio.Slider(visible=False),
            gradio.Dropdown(visible=False, value=None, choices=None),
            gradio.Slider(visible=False, value=None),
        )
    if is_video(facefusion.globals.target_path):
        output_video_resolution = detect_video_resolution(
            facefusion.globals.target_path
        ...
        facefusion.globals.output_video_fps = 30  # Default value
        # detect_video_fps(
        #     facefusion.globals.target_path
        # )
        return (
            ...
            gradio.Slider(visible=True, value=facefusion.globals.output_video_fps),
        )
    return (
        gradio.Slider(visible=False),
        gradio.Dropdown(visible=False, value=None, choices=None),
