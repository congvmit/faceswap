...
def render() -> None:
    ...
    WEBCAM_IMAGE = gradio.Image(label=wording.get("uis.webcam_image"))
...
def listen() -> None:
    ...
    if webcam_mode_radio and webcam_resolution_dropdown and webcam_fps_slider:
        start_event = WEBCAM_START_BUTTON.click(
            start,
            inputs=[webcam_mode_radio, webcam_resolution_dropdown, webcam_fps_slider],
...
def start(
    webcam_mode: WebcamMode, webcam_resolution: str, webcam_fps: Fps
    ...
    if webcam_capture and webcam_capture.isOpened():
        ...
        for capture_frame in multi_process_capture(
            source_face, webcam_capture, webcam_fps
        ):
...
def multi_process_capture(
    source_face: Face, webcam_capture: cv2.VideoCapture, webcam_fps: Fps
) -> Generator[VisionFrame, None, None]:
    with tqdm(
        ...
        with ThreadPoolExecutor(
            max_workers=facefusion.globals.execution_thread_count
        ) as executor:
            futures = []
            ...
            while webcam_capture and webcam_capture.isOpened():
                ...
                future = executor.submit(
                    process_stream_frame, source_face, capture_frame
                )
...
def process_stream_frame(
    source_face: Face, target_vision_frame: VisionFrame
) -> VisionFrame:
...
def open_stream(
    stream_mode: StreamMode, stream_resolution: str, stream_fps: Fps
    ...
    if stream_mode == "udp":
        commands.extend(
            ["-b:v", "2000k", "-f", "mpegts", "udp://localhost:27000?pkt_size=1316"]
        )
    if stream_mode == "v4l2":
        try:
            ...
            logger.error(
                wording.get("stream_not_loaded").format(stream_mode=stream_mode),
                __name__.upper(),
            )
