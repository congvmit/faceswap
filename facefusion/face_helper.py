...
def warp_face_by_bounding_box(
    temp_vision_frame: VisionFrame, bounding_box: BoundingBox, crop_size: Size
) -> Tuple[VisionFrame, Matrix]:
    ...
    target_points = numpy.array([[0, 0], [crop_size[0], 0], [0, crop_size[1]]]).astype(
        numpy.float32
    )
    affine_matrix = cv2.getAffineTransform(source_points, target_points)
    if (
        bounding_box[2] - bounding_box[0] > crop_size[0]
        or bounding_box[3] - bounding_box[1] > crop_size[1]
    ):
        interpolation_method = cv2.INTER_AREA
    else:
        interpolation_method = cv2.INTER_LINEAR
    crop_vision_frame = cv2.warpAffine(
        temp_vision_frame, affine_matrix, crop_size, flags=interpolation_method
    )
    return crop_vision_frame, affine_matrix
...
def create_bounding_box_from_face_landmark_68(
...
def distance_to_bounding_box(
    points: numpy.ndarray[Any, Any], distance: numpy.ndarray[Any, Any]
) -> BoundingBox:
...
def distance_to_face_landmark_5(
    points: numpy.ndarray[Any, Any], distance: numpy.ndarray[Any, Any]
) -> FaceLandmark5:
