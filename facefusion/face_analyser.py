    ...
    with THREAD_LOCK:
        ...
        if FACE_ANALYSER is None:
            if facefusion.globals.face_detector_model in ["many", "retinaface"]:
                face_detectors["retinaface"] = onnxruntime.InferenceSession(
            ...
            if facefusion.globals.face_detector_model in ["many", "scrfd"]:
                face_detectors["scrfd"] = onnxruntime.InferenceSession(
                    MODELS.get("face_detector_scrfd").get("path"),
                    providers=apply_execution_provider_options(
                        facefusion.globals.execution_providers
                    ),
                )
            if facefusion.globals.face_detector_model in ["many", "yoloface"]:
                face_detectors["yoloface"] = onnxruntime.InferenceSession(
            ...
            if facefusion.globals.face_detector_model in ["yunet"]:
                face_detectors["yunet"] = cv2.FaceDetectorYN.create(
                    MODELS.get("face_detector_yunet").get("path"), "", (0, 0)
                )
            ...
            face_landmarkers["68"] = onnxruntime.InferenceSession(
                MODELS.get("face_landmarker_68").get("path"),
                providers=apply_execution_provider_options(
                    facefusion.globals.execution_providers
                ),
            )
            face_landmarkers["68_5"] = onnxruntime.InferenceSession(
...
def detect_with_retinaface(
    ...
    for index, feature_stride in enumerate(feature_strides):
        keep_indices = numpy.where(
            detections[index] >= facefusion.globals.face_detector_score
        )[0]
        if keep_indices.any():
            ...
            face_landmark_5_raw = (
                detections[index + feature_map_channel * 2] * feature_stride
            )
...
def detect_with_yunet(
    ...
    if numpy.any(detections):
        for detection in detections:
            ...
            face_landmark_5_list.append(
                detection[4:14].reshape((5, 2)) * [ratio_width, ratio_height]
            )
    ...
    if facefusion.globals.face_detector_score > 0:
        ...
        for index in keep_indices:
            ...
            embedding, normed_embedding = calc_embedding(
                vision_frame, landmarks.get("5/68")
            )
...
def expand_face_landmark_68_from_5(face_landmark_5: FaceLandmark5) -> FaceLandmark68:
    ...
    face_landmark_68_5 = face_landmarker.run(
        None, {face_landmarker.get_inputs()[0].name: [face_landmark_5]}
    )[0][0]
...
def get_average_face(
    ...
    """Get the average face from a list of vision frames.
    In this released version, only the first face is used.
    """
    average_face = None
    faces = []
    embedding_list = []
...
def get_many_faces(vision_frame: VisionFrame) -> List[Face]:
    ...
    try:
        ...
        if faces_cache:
            ...
            if facefusion.globals.face_detector_model in ["many", "retinaface"]:
                (
                    bounding_box_list_retinaface,
                    face_landmark_5_list_retinaface,
                    score_list_retinaface,
                ) = detect_with_retinaface(
                    vision_frame, facefusion.globals.face_detector_size
                )
            ...
            if facefusion.globals.face_detector_model in ["many", "scrfd"]:
                (
                    bounding_box_list_scrfd,
                    face_landmark_5_list_scrfd,
                    score_list_scrfd,
                ) = detect_with_scrfd(
                    vision_frame, facefusion.globals.face_detector_size
                )
            ...
            if facefusion.globals.face_detector_model in ["many", "yoloface"]:
                (
                    bounding_box_list_yoloface,
                    face_landmark_5_list_yoloface,
                    score_list_yoloface,
                ) = detect_with_yoloface(
                    vision_frame, facefusion.globals.face_detector_size
                )
            ...
            if facefusion.globals.face_detector_model in ["yunet"]:
                (
                    bounding_box_list_yunet,
                    face_landmark_5_list_yunet,
                    score_list_yunet,
                ) = detect_with_yunet(
                    vision_frame, facefusion.globals.face_detector_size
                )
            ...
            if bounding_box_list and face_landmark_5_list and score_list:
                faces = create_faces(
                    vision_frame, bounding_box_list, face_landmark_5_list, score_list
                )
            if faces:
...
def find_similar_faces(
    reference_faces: FaceSet, vision_frame: VisionFrame, face_distance: float
) -> List[Face]:
...
def sort_by_order(faces: List[Face], order: FaceAnalyserOrder) -> List[Face]:
    ...
    if order == "small-large":
        return sorted(
            faces,
            key=lambda face: (face.bounding_box[2] - face.bounding_box[0])
