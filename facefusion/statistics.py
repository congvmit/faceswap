...
def create_statistics(static_faces: FaceSet) -> Dict[str, Any]:
    ...
    for faces in static_faces.values():
        statistics["total_frames_with_faces"] = (
            statistics.get("total_frames_with_faces") + 1
        )
        for face in faces:
            ...
            if numpy.array_equal(face.landmarks.get("5"), face.landmarks.get("5/68")):
                statistics["total_face_landmark_5_fallbacks"] = (
                    statistics.get("total_face_landmark_5_fallbacks") + 1
                )
    ...
    if face_detector_score_list:
        ...
        statistics["average_face_detector_score"] = round(
            numpy.mean(face_detector_score_list), 2
        )
    if face_landmarker_score_list:
        statistics["min_face_landmarker_score"] = round(
            min(face_landmarker_score_list), 2
        )
        statistics["max_face_landmarker_score"] = round(
            max(face_landmarker_score_list), 2
        )
        statistics["average_face_landmarker_score"] = round(
            numpy.mean(face_landmarker_score_list), 2
        )
    return statistics
