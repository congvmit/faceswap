...
def get_voice_extractor() -> Any:
    ...
    with THREAD_LOCK:
        ...
        if VOICE_EXTRACTOR is None:
            ...
            VOICE_EXTRACTOR = onnxruntime.InferenceSession(
                model_path,
...
def extract_voice(temp_audio_chunk: AudioChunk) -> AudioChunk:
    ...
    trim_size = 3840
    temp_audio_chunk, pad_size = prepare_audio_chunk(
        temp_audio_chunk.T, chunk_size, trim_size
    )
    temp_audio_chunk = decompose_audio_chunk(temp_audio_chunk, trim_size)
    with THREAD_SEMAPHORE:
        temp_audio_chunk = voice_extractor.run(
            None, {voice_extractor.get_inputs()[0].name: temp_audio_chunk}
        )[0]
    temp_audio_chunk = compose_audio_chunk(temp_audio_chunk, trim_size)
    temp_audio_chunk = normalize_audio_chunk(
        temp_audio_chunk, chunk_size, trim_size, pad_size
    )
    return temp_audio_chunk


def prepare_audio_chunk(
    temp_audio_chunk: AudioChunk, chunk_size: int, trim_size: int
    ...
    temp_audio_chunk = (
        temp_audio_chunk.astype(numpy.float32) / numpy.iinfo(numpy.int16).max
    )
    temp_audio_chunk = numpy.pad(
        temp_audio_chunk, ((0, 0), (trim_size, trim_size + pad_size))
    )
    temp_audio_chunks = []
...
def decompose_audio_chunk(temp_audio_chunk: AudioChunk, trim_size: int) -> AudioChunk:
    ...
    temp_audio_chunk = scipy.signal.stft(
        temp_audio_chunk, nperseg=frame_size, noverlap=frame_overlap, window=window
    )[2]
    temp_audio_chunk = numpy.stack(
        (numpy.real(temp_audio_chunk), numpy.imag(temp_audio_chunk)), axis=-1
    ).transpose((0, 3, 1, 2))
    temp_audio_chunk = temp_audio_chunk.reshape(
        -1, 2, 2, trim_size + 1, voice_extractor_shape[3]
    ).reshape(-1, voice_extractor_shape[1], trim_size + 1, voice_extractor_shape[3])
...
def compose_audio_chunk(temp_audio_chunk: AudioChunk, trim_size: int) -> AudioChunk:
    ...
    temp_audio_chunk = numpy.pad(
        temp_audio_chunk,
        ((0, 0), (0, 0), (0, trim_size + 1 - voice_extractor_shape[2]), (0, 0)),
    )
    temp_audio_chunk = temp_audio_chunk.reshape(
        -1, 2, trim_size + 1, voice_extractor_shape[3]
    ).transpose((0, 2, 3, 1))
    temp_audio_chunk = temp_audio_chunk[:, :, :, 0] + 1j * temp_audio_chunk[:, :, :, 1]
    temp_audio_chunk = scipy.signal.istft(
        temp_audio_chunk, nperseg=frame_size, noverlap=frame_overlap, window=window
    )[1]
    temp_audio_chunk *= numpy.sqrt(1.0 / window.sum() ** 2)
    return temp_audio_chunk
...
def normalize_audio_chunk(
    temp_audio_chunk: AudioChunk, chunk_size: int, trim_size: int, pad_size: int
) -> AudioChunk:
